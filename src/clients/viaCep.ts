...
const ViaCepResourceConfig: IConfig = config.get('App.resources.ViaCep')
...
export class ViaCep {
  public async fetchCep(cep: number): Promise<typeof viaCepObj | undefined> {
    try {
      const response = await axios.get<typeof viaCepObj>(
        `${ViaCepResourceConfig.get('apiUrl')}/${cep}/json/`
      )
